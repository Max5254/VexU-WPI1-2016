...
bool drivePID(int mm) {
	...
	do{
		...
		straight.error=straight.target -(SensorValue[driveEncL]+	SensorValue[driveEncR]);
		angle.error=(SensorValue[driveEncL]-SensorValue[driveEncR]); //target is 0 so lazy
		...
		writeDebugStreamLine("%d,%d,%d",(straight.error)-130*sgn(mm),SensorValue[driveEncL],SensorValue[driveEncR]);
		delay(50);
		...
		vel = (((float)(SensorValue[driveEncL] - prevDist))/((float)(nSysTime- velTime)));
		prevDist = SensorValue[driveEncL];
		velTime = nSysTime;
...
bool turnPID(float degrees) {
	...
	do{
		...
		angle.error=angle.target-(SensorValue[driveEncR]-SensorValue[driveEncL]); //target is 0 so lazy
